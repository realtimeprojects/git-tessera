...
class GitTessera:
  ...
  def __init__(self):
    self.git = git.Repo(".")
    Tessera._tesserae = "%s/.tesserae"%self.git.working_dir
  ...
  def cmd_edit(self, args):
    ...
    if self.git.is_dirty():
      for tessera_path in tessera_paths:
        t = Tessera(tessera_path)
        self.git.index.add(["%s/tessera"%tessera_path])
        self.git.index.commit("tessera updated: %s"%t.title)
    return True
  ...
  def cmd_create(self, args):
    ...
    self.git.index.commit("tessera created: %s"%t.title)
    return True
